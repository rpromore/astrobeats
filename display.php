...
function lastfm() {
	...
	foreach( $lastfm->tracks->track AS $i )
		$items[] = array("name" => $i->name, "image" => !empty($i->image) ? $i->image[3]->{'#text'} : "", "from" => "lastfm");
	return $items;
}
function reddit() {
	$items = array();
...
$reddit = json_decode(file_get_contents("http://reddit.com/r/listentothis.json", true));

foreach( $reddit->data->children AS $i )
	$items[] = array("name" => $i->data->title, "image" => !empty($i->data->media->oembed->thumbnail_url) ? $i->data->media->oembed->thumbnail_url : "", "from" => "reddit");
	
$soundcloud = json_decode(file_get_contents("http://api.soundcloud.com/tracks.json?client_id=c6dc5b166e3d58345cc4751665f9ce08&order=hotness", true));
foreach( $soundcloud AS $i )
	$items[] = array("name" => $i->title, "image" => str_replace("large.jpg", "t300x300.jpg", $i->user->avatar_url), "from" => "soundcloud");
